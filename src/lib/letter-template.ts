...
- "write to [the surgeon/referrer's name]" / "pre-operative assessment letter" / "peri-operative assessment letter" / "write to Dr [X]" where the context is a surgical pre-op assessment -> this means a formal pre-operative medical assessment letter. Use EXACTLY this structure, do not invent a different format:
  ...
  - Then these UPPERCASE section headings in this exact order, each on its own plain line (omit a section only if truly nothing applies, but prefer including it with "[not documented — please verify]"): PRESENTING HISTORY, PAST MEDICAL & SURGICAL HISTORY, CURRENT MEDICATIONS (grouped with bold subheadings like Cardiac, Anticoagulation, Endocrine, etc.), ALLERGIES / INTOLERANCES, SOCIAL HISTORY, RECENT INVESTIGATIONS, SYSTEMS REVIEW, ANAESTHETIC / SURGICAL RISK ASSESSMENT, PRE-OPERATIVE PLAN (numbered items, always covering anticoagulation/medication withholding/fasting instructions/pre-op investigations/post-op follow-up), FITNESS FOR SURGERY.
...
- If a section has nothing documented, OMIT THE SECTION ENTIRELY for non-pre-op formats (GP letters, referral letters, 132 letters, ward round notes, patient instructions). Do NOT write "[not documented]", "[not stated]", or "[please verify]" for these formats — just leave the section out. For pre-operative assessment letters only, you may write "[not documented — please verify]" for safety-critical sections such as allergies and surgical history.
